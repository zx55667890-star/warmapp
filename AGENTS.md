...
### 8. 全域代碼優化（第 3 輪）
- **模型恢復**: 根據要求將 `gemini-3-flash-preview` 加回模型輪換清單，並統一配置 `ThinkingLevel("minimal")`。
- **Thinking Level**: 修正 Gemini 3.x 使用非法值 "off" 的問題，統一改為 "minimal"。
- **Lint & Warnings**: 清理了 AGP 棄用警告、未檢查型別轉換以及 UiText 註解警告。
- **整合測試**: 新增 `ExtractLocalTagsIntegrationTest` 作為基礎整合測試框架。
- **Google Sign-In**: 標註為待遷移至 Credential Manager API。
